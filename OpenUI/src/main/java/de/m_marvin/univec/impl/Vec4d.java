...
public class Vec4d implements IVector4Math<Double, Vec4d, IVector4<? extends Number>> {
	...
	@Override
	public Vec4d cross(IVector4<? extends Number> vec) {
		return new Vec4d(
				this.w * (double) vec.z() - this.z * (double) vec.w(),
				this.z * (double) vec.x() - this.x * (double) vec.z(),
				this.x * (double) vec.y() - this.y * (double) vec.x(),
				this.y * (double) vec.w() - this.w * (double) vec.x()
			);
	}
	...
	public int hashCode() {
		...
		result = result * 32 + Double.hashCode(this.w);
		return result;
	}
