...
public class Vec4f implements IVector4Math<Float, Vec4f, IVector4<? extends Number>> {
	...
	@Override
	public Vec4f cross(IVector4<? extends Number> vec) {
		return new Vec4f(
				this.w * (float) vec.z() - this.z * (float) vec.w(),
				this.z * (float) vec.x() - this.x * (float) vec.z(),
				this.x * (float) vec.y() - this.y * (float) vec.x(),
				this.y * (float) vec.w() - this.w * (float) vec.x()
			);
	}
	...
	public int hashCode() {
		...
		result = result * 32 + Float.hashCode(this.w);
		return result;
	}
