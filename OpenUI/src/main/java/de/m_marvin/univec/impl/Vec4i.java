...
/*
 * Implementation of a 3 dimensional int vector
 */
public class Vec4i implements IVector4Math<Integer, Vec4i, IVector4<? extends Number>> {
	...
	public Vec4i(Integer x, Integer y, Integer z, int w) {
		this.x = x;
		this.y = y;
		this.z = z;
		this.w = w;
	...
	@Override
	public double angle(IVector4<? extends Number> vec) {
	...
	@Override
	public Vec4i cross(IVector4<? extends Number> vec) {
		return new Vec4i(
				this.w * (int) vec.z() - this.z * (int) vec.w(),
				this.z * (int) vec.x() - this.x * (int) vec.z(),
				this.x * (int) vec.y() - this.y * (int) vec.x(),
				this.y * (int) vec.w() - this.w * (int) vec.x()
			);
	}
	...
	public int hashCode() {
		...
		result = result * 32 + Integer.hashCode(this.w);
		return result;
	}
